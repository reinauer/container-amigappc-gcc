...
Build ADTools ppc-amigaos-gcc on Ubuntu Linux.
...
Options:
  ...
  --jobs N                 Parallel make jobs (default: Linux CPU count)
  ...
  --skip-python-venv       Use system Python Mako instead of a local venv
...
Examples:
  ...
  ./build_linux.sh --gcc-branch 8 --binutils-branch 2.23.2
EOF
}
...
ensure_python_env() {
  if [[ "$INSTALL_PYTHON_VENV" -eq 0 ]]; then
    log "Using system Python Mako"
    python3 -c 'import mako' >/dev/null 2>&1 || \
      die "Python Mako not found; install python3-mako or rerun without --skip-python-venv"
    return
  fi
...
ensure_prefix_writable() {
  ...
  if mkdir -p "$prefix" 2>/dev/null; then
    ...
    die "prefix is not writable: ${prefix}. Create it and chown it to $(id -un), or choose a writable --prefix"
  fi
...
build_cross() {
  ...
  log "Building ADTools ppc-amigaos-gcc ${ADTOOLS_GCC_VERSION}"
